...
class category:
    def __init__(self,root):
        ...
        # title
        lbl_title=Label(self.root,text="Manage Product Category",font=("goudy old style",30,"bold"),bg="#184a45",fg="white",bd=3,relief=RIDGE).pack(side=TOP,fill=X,padx=10,pady=20)

        lbl_name=Label(self.root,text="Enter Category Name",font=("goudy old style",30,"bold"),bg="white").place(x=50,y=100)
        txt_name=Entry(self.root,textvariable=self.var_name,font=("goudy old style",18,"bold"),bg="lightyellow").place(x=50,y=170,width=300)
        ...
        self.im1_label=Label(self.root,image=self.im1,bd=3,relief=RAISED).place(x=50,y=220)
        
        self.im2=Image.open("images/category.jpg")
        self.im2=self.im2.resize((500,250),Image.LANCZOS)
        self.im2=ImageTk.PhotoImage(self.im2)

        self.im2_label=Label(self.root,image=self.im2,bd=3,relief=RAISED).place(x=580,y=220)
    ...
    # Function to display category data in the Treeview
    def show(self):
        con = sqlite3.connect(database="ims.db")
        cur = con.cursor()
        try:
